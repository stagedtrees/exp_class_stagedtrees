...
ROC_CURVE_NO_CUTOFF <- readRDS("ROC_CURVE_NO_CUTOFF.rds")
...
interpolated_sensitivity_no_cutoff <- array(
  data = rep(NA, 120),
  dim = c(
    length(datasets),
    length(classifiers),
...
for(i in 1:length(datasets)) {
  for(j in 1:length(classifiers)) {
    for(k in 1:nreps) {
      interpolated_sensitivity_no_cutoff[i, j, k, ] <- interpolation(ROC_CURVE_NO_CUTOFF[1, i, j, k, ], 
                                                                     ROC_CURVE_NO_CUTOFF[2, i, j, k, ])
    }
  }
}
...
AVG_ROC_CURVE_NO_CUTOFF <- apply(interpolated_sensitivity_no_cutoff, c(1,2,4), mean)
saveRDS(AVG_ROC_CURVE_NO_CUTOFF, "AVG_ROC_CURVE_NO_CUTOFF.rds")



pdf("roc_curves1_no_cutoff.pdf")
par(mfrow = c(2, 2))
colors <- rainbow(length(classifiers) - 1)
for(j in 1:4) {
  plot(fixed_specificities, AVG_ROC_CURVE_NO_CUTOFF[j, 1, ], lwd = 0.5,
       type = "l", xlab = "1 - Specificity", ylab = "Sensitivity", 
       main = attr(AVG_ROC_CURVE_NO_CUTOFF, "dimnames")$data[j], xlim = c(0, 1), ylim = c(0, 1))
  for(i in 2:length(classifiers)) {
    points(fixed_specificities, AVG_ROC_CURVE_NO_CUTOFF[j, i, ], lwd = 0.5, col = colors[i-1], type = "l")
  }
}
dev.off()


pdf("roc_curves2_no_cutoff.pdf")
par(mfrow = c(2, 2))
colors <- rainbow(length(classifiers) - 1)
for(j in 5:8) {
  plot(fixed_specificities, AVG_ROC_CURVE_NO_CUTOFF[j, 1, ], lwd = 0.5,
...
## plot accuracies
ggplot(data = data_no_cutoff[stat %in% c("accuracy", "balanced_accuracy") & classifier %in% classifiers], aes(y = data, x = value, 
  ...
  ggsave("plot_accuracy_no_cutoff.pdf", width = 7, height = 6, units = "in")



## plot time
ggplot(data = data_no_cutoff[stat == "time" & classifier %in% classifiers], aes(y = data, x = value, 
                                                                      group = classifier, color = classifier)) + 
  ...
  xlab("seconds") + 
  ggsave("plot_time_no_cutoff.pdf", width = 5, height = 6, units = "in")


## plot spec - sens - fp - fn
ggplot(data = data_no_cutoff[stat %in% c("sens", "fn", "spec", "fp") & classifier %in% classifiers], aes(y = data, x = value, 
                                                                                               group = classifier, color = classifier)) + 
  geom_jitter(height = 0.2, width = 0, alpha = 0.5) + facet_grid(cols = vars(stat), scales = "free") + 
  theme_bw() +
  theme(
  ...
  ggsave("plot_confusion_matrix_no_cutoff.pdf", width = 7, height = 6, units = "in")




## plot auc - cutoff - precision - f1
ggplot(data = data_no_cutoff[stat %in% c("auc", "cutoff", "precision", "f1") & classifier %in% classifiers], aes(y = data, x = value, 
                                                                                                       group = classifier, color = classifier)) + 
  geom_jitter(height = 0.2, width = 0, alpha = 0.5) + facet_grid(cols = vars(stat), scales = "free") + 
  theme_bw() +
  theme(
  ...
  ggsave("plot_auc_cutoff_precision_f1_no_cutoff.pdf", width = 7, height = 6, units = "in")
