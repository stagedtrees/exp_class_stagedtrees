    ...
    for (c_name in classifiers) {
      ...
      if (file.exists(filename)){
        ...
        for (stat in statistics[-length(statistics)]){
          stat_fun <- get(stat)
          TABLE_NO_CUTOFF[stat, d, c_name, r] <- stat_fun(res, true)
        }
        stat_fun <- get(statistics[length(statistics)])
        auxiliar <- matrix(unlist(stat_fun(res, true)), nrow = 2, byrow = TRUE)
        ROC_CURVE_NO_CUTOFF[1:2, d, c_name, r, 1:NCOL(auxiliar)] <- auxiliar
      }
    }
  }
}

saveRDS(TABLE_NO_CUTOFF, "TABLE_NO_CUTOFF.rds")
saveRDS(ROC_CURVE_NO_CUTOFF, "ROC_CURVE_NO_CUTOFF.rds")
